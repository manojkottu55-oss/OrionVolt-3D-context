...
const GlowCursor = ({
  ...
  useEffect(() => {
    ...
    // Optional touch screen check so we don't init WebGL on mobile for cursor
    ...
    const updatePointer = event => {
      ...
      const y = window.innerHeight - event.clientY; // WebGL y is bottom-up
  ...
  return (
    <div ref={containerRef} className={`glow-cursor${className ? ` ${className}` : ''}`} style={{ ...style, position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 9999 }} {...rest}>
      <canvas ref={canvasRef} className="glow-cursor__canvas" style={{ mixBlendMode: blendMode, width: '100%', height: '100%' }} aria-hidden="true" />
      {children && <div className="glow-cursor__content" style={{ pointerEvents: 'auto' }}>{children}</div>}
    </div>
  );
};
